  ...
  public SolarEventCalculator(Location location, TimeZone timeZone) {
   ...
   public String computeSunriseTime(Zenith solarZenith, Calendar date) {
    ...
    private BigDecimal getBaseLongitudeHour() {
        return divideBy(BigDecimal.valueOf(location.getLongitude()), BigDecimal.valueOf(15));
    }
    ...
    private BigDecimal getCosineSunLocalHour(BigDecimal sunTrueLong, Zenith zenith) {
        ...
        BigDecimal sinLatitude = BigDecimal.valueOf(Math.sin(convertDegreesToRadians(BigDecimal.valueOf(location.getLatitude())).doubleValue()));
        BigDecimal cosLatitude = BigDecimal.valueOf(Math.cos(convertDegreesToRadians(BigDecimal.valueOf(location.getLatitude())).doubleValue()));
